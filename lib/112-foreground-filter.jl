...
function go(project_file, config_file)
    ...
    if isinf(config.threshold)
        whiten_noise(project, config)
    else
        foreground_filter(project, config)
    end
end

function whiten_noise(project, config)
    ...
    BPJSpec.whiten_noise!(output_mmodes, output_transfermatrix, output_covariance,
                          output_noise_whitener,
                          input_mmodes, input_transfermatrix, input_noisematrix,
                          input_foregroundmatrix,
                          tempdir=joinpath(path, "tmp"), cleanup=true)
end
