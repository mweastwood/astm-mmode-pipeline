function wsclean(name; weight="uniform", minuvw=0)
    if endswith(name, "ms")
        ms_name = name
    ...
    readstring(`wsclean -j 1 -size 2048 2048 -scale 0.0625 -weight $weight -minuv-l $minuvw -name $name $ms_name`)
...
function wsclean(input, output; weight="uniform", minuvw=0)
    readstring(`wsclean -j 1 -size 2048 2048 -scale 0.0625 -weight $weight -minuv-l $minuvw -name $output $input`)
