...
#    Ntime2 = size(data, 3) # number of integrations in the data
#    Nbase = size(data, 2)  # number of baselines
...
#    N = length(data_list) # the number of days
#    M = size(data_list[1], 2) # the number of baselines
#
...
#    N = size(data_list[1], 2) # the number of baselines
#    output = 0.0
#    for (data, flags) in zip(data_list, flag_list)
#        for idx = 1:N
#            output += ifelse(flags[idx], 0.0, abs2(data[1, idx]))
#            output += ifelse(flags[idx], 0.0, abs2(data[2, idx]))
#        end
#    end
#    output
#end
...
#    for (data, flags) in zip(data_list, flag_list)
#        for idx = 1:N
#            output[idx] += 0.5*ifelse(flags[idx], 0.0, data[1, idx])
#            output[idx] += 0.5*ifelse(flags[idx], 0.0, data[2, idx])
#        end
#    end
#    output
#end
