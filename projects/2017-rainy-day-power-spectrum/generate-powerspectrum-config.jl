...
function main()
    ...
    # "sidereal-noise" => noise varies sidereally
    # "constant-noise" => noise is constant with time
    noise_errors = ("sidereal-noise", "constant-noise")
    ...
    open(joinpath(@__DIR__, "AutoGenerated.mk"), "w") do makefile
        ...
        for process in processing
            ...
            # Mess with noise
            for noise in noise_errors
                create_301_mess_with_noise_yml(makefile, process, noise)
                create_030_getmmodes_with_noise_yml(makefile, process, noise)
                create_101_average_channels_yml(makefile, process, noise)
                create_103_full_rank_compress_yml(makefile, process, noise)
                for filter in filtering
                    create_112_foreground_filter_yml(makefile, process, noise, filter)
                    for estimate in estimator
                        if process == "peeled"
                            create_121_fisher_matrix_yml(makefile, noise, filter, estimate)
                        end
                        create_122_quadratic_estimator_yml(makefile, process, noise, filter, estimate)
                    end
                end
            end
        end
...
function create_030_getmmodes_with_noise_yml(makefile, process, noise)
    filename = "030-getmmodes-$process-$noise.yml"
    open(joinpath(temp, filename), "w") do file
        println(file,
                """$HEADER
                input: 032-predicted-visibilities-$process-$noise
    ...
    println(makefile,
            """.pipeline/030-m-modes-$process-$noise: \\
            \t\t\$(LIB)/030-getmmodes.jl project.yml $dirname/$filename \\
            \t\t.pipeline/032-predicted-visibilities-$process-$noise \\
            \t\t.pipeline/100-transfer-matrix
            \t\$(call launch-remote,1)
            """)
end
...
function create_301_mess_with_noise_yml(makefile, process, noise)
    filename = "301-mess-with-noise-$process-$noise.yml"
    constant = noise == "constant-noise"

    open(joinpath(temp, filename), "w") do file
        println(file,
                """$HEADER
                input: 032-predicted-visibilities-$process
                output: 032-predicted-visibilities-$process-$noise
                hierarchy: hierarchy
                sefd: xxx-sefd-as-a-function-of-time.jld
                constant: $constant
                """)
    end
    replace_if_different(filename)
    ...
    println(makefile,
            """.pipeline/032-predicted-visibilities-$process-$error: \\
            \t\t\$(LIB)/301-mess-with-noise.jl project.yml $dirname/$filename \\
            \t\t.pipeline/032-predicted-visibilities-$process
            \t\$(launch)
            """)
end
